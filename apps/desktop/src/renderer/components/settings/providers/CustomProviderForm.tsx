...
import type { ConnectedProvider, CustomCredentials } from '@accomplish/shared';
import {
  ModelSelector,
  ConnectButton,
  ConnectedControls,
...
export function CustomProviderForm({
  ...
  const [providerName, setProviderName] = useState('My Custom Provider');
        ...
        <AnimatePresence mode="wait">
          {!isConnected ? (
            ...
            >
              ...
              <div>
                ...
                <input
                  ...
                  data-testid="custom-server-url"
                  className="w-full rounded-md border border-input bg-background px-3 py-2.5 text-sm"
                />
                <p className="mt-1 text-xs text-muted-foreground">
                  Base URL for your OpenAI-compatible API (e.g., http://localhost:8000/v1)
                </p>
              </div>
            ...
            >
              ...
              <div className="space-y-3">
                <div>
                  <label className="mb-2 block text-sm font-medium text-foreground">Provider Name</label>
                  <input
                    type="text"
                    value={(connectedProvider?.credentials as CustomCredentials)?.providerName || 'Custom Provider'}
                    disabled
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={(connectedProvider?.credentials as CustomCredentials)?.serverUrl || 'http://localhost:8000'}
                    disabled
                    className="w-full rounded-md border border-input bg-muted/50 px-3 py-2.5 text-sm text-muted-foreground"
                  />
                </div>
